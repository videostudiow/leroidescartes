...
export default async function CollectionsPage({ params }: { params: { locale: string } }) {
  ...
  try {
    collections = await getAllCollections(getShopContext(locale));
  } catch {
      ...
      <div className="bg-dark text-white py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="section-badge text-secondary">
            <span>{dict.collections.badge.toUpperCase()}</span>
          </div>
          <h1 className="font-heading text-[clamp(3rem,7vw,6rem)] font-black uppercase leading-[0.9]">
            {dict.collections.titre1}<br />{dict.collections.titre2}
          </h1>
        </div>
      </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {PLACEHOLDERS.map((col) => (
        ...
        >
          ...
          >
            <span className="font-heading font-black text-5xl" style={{ color: col.color }}>
              {col.label.charAt(0)}
            </span>
          </div>
          <div className="p-5 flex items-center justify-between">
